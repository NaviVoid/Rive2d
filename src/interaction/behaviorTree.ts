...
export class BehaviorTreeBuilder {
  ...
  build(graph: ModelGraph, route: ResolvedMotionRoute): BehaviorTree {
    ...
    const nodes: BehaviorNode[] = [
      new ConditionNode(`${route.key}:eligible`, context => entry !== null && this.conditions.evaluate(entry, context.state)),
    ];
    ...
    if (entry) nodes.push(new AnimationNode(`${route.key}:animation`, route));
    if (executeCommands && entry?.postCommand) {
      nodes.push(new CommandNode(`${route.key}:post-command`, entry.postCommand, this.commandRuntime));
    }
