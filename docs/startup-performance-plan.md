...
- 在缓存根目录增加持久化索引，以 `规范化 LPK 路径 + 文件长度 + 修改时间` 快速命中已完成缓存。
- 只有索引缺失、源文件元数据变化或缓存标记缺失时，才回退到完整 MD5 和重新检查缓存目录。
- 在首次解密展开阶段预处理模型 JSON 和 motion JSON；协议请求只读取已经修正的缓存文件。
- 保留协议层的兼容性兜底，但命中版本化缓存时不再重复解析和序列化 motion JSON。
- Rust 只负责归档、解密、缓存和字节传输；Live2D motion 对象仍由 WebView 引擎创建。

本轮不使用 Rust 重写 Cubism motion 创建，也不把所有动作重新编码成私有二进制格式，避免破坏不同模型格式的兼容性。
...
- 当前启动阶段不再同步等待全部点击区域动作；交互动作通过统一资源缓存按需加载。
- 当前 Idle/init 链可达的 `NextMtn`、`PostCommand` 和 `start_mtn` 目标。
...
1. [x] 引入 `MotionResourceCache`，统一加载任务、状态和取消逻辑。
2. [x] 从模型图构建启动动作依赖闭包，增加循环和缺失引用处理。
3. [x] 将启动阶段从“全部 motion”改为当前初始 Idle 和 init 链。
4. [ ] 增加有界后台预热和交互优先级调度。
5. [x] 增加 Rust 持久化 LPK 缓存索引，减少跨进程完整 MD5。
6. [x] 在 Rust 解压阶段预处理模型和 motion JSON，移除协议请求中的重复解析。
7. [ ] 增加关键音频的独立预热和非阻塞播放策略。
8. [ ] 使用同一模型重复启动、冷缓存启动、模型切换和交互中断进行对比测试。
...
同一模型的真实启动日志从等待 124 个 motion 的约 44.6 秒，降为：

```text
resources-ready             2 ms
engine-model-ready       2480 ms
critical-preload            66 ms  (requested=1, loaded=1)
startup-load-complete     2549 ms
```

首次按需动作加载在测试中约 23-38 ms；后续动作由 `MotionResourceCache` 复用加载结果。剩余未完成项是有界后台预热、独立音频预热和更完整的冷缓存/切换场景对比测试。
